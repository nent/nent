...
The contained HTML is parsed before rendering and special handling is given if any child elements are [\<n-view-prompt\>](/components/n-view-prompt) elements. Before rendering its own HTML, this component iterates the collection and evaluates their conditions (_when_ attribute) looking for the first [\<n-view-prompt\>](/components/n-view-prompt) that should be displayed, using the order they are declared. If and when a non-visited [\<n-view-prompt\>](/components/n-view-prompt) is found, its route is activated and subsequently marked as visited.
