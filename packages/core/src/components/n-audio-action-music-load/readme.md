...
This component declares audio used within this [\<n-view-prompt\>](/components/n-view-prompt) route. The [\<n-audio-action-music-load\>](/components/n-audio-action-music-load) represents audio files and play behaviors. They are sent to the audio player to pre-load or play when the route is active. The player manages them according to their settings.
