...
export class ViewDoService {
  ...
  public async captureChildElements(el: HTMLElement = this.el) {
    debugIf(
      this.debug,
      `x-app-view-do: ${this.route.path} resolve children called`,
    )
  ...
  public back(element: string, eventName: string) {
    debugIf(
      this.debug,
      `x-app-view-do: ${this.route.path} back fired from ${element}:${eventName}`,
    )
    this.cleanup()
    this.route.goBack()
  }
  ...
  public async next(
    ...
    debugIf(
      this.debug,
      `x-app-view-do: ${this.route.path} next fired from ${element}:${eventName}`,
    )
