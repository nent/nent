...
export function captureXNextClickEvent(
  ...
  captureElementsEventOnce<HTMLElement, MouseEvent>(
    ...
    'click',
    (el: HTMLElement, _e: MouseEvent) => {
      const route = el.getAttribute('n-next')
...
export function captureXLinkClickEvent(
  ...
  captureElementsEventOnce<HTMLElement, MouseEvent>(
    ...
    'click',
    (el: HTMLElement, _e: MouseEvent) => {
      const route = el.getAttribute('n-link')
      eventHandler(el.localName, route)
    },
  )
}
...
export function resolveElementChildTimedNodesByTime(
  ...
  timedNodes?.forEach(node => {
    if (
      ...
      debugIf(
        debug,
        `n-view-prompt: node ${node.element.id} is after start: ${node.start} before end: ${node.end}`,
      )
      ...
      if (
        ...
        debugIf(
          debug,
          `n-view-prompt: node ${node.element.id} is after start: ${node.start} before end: ${node.end} [adding classIn: ${node.classIn}]`,
        )
      ...
      if (node.element.hasAttribute('hidden')) {
        debugIf(
          debug,
          `n-view-prompt: node ${node.element.id} is after start: ${node.start} before end: ${node.end} [removing hidden attribute]`,
        )
    ...
    if (node.end > -1 && time > node.end) {
      ...
      debugIf(
        debug,
        `n-view-prompt: node ${node.element.id} is after end: ${node.end}`,
      )
      if (
        node.classIn &&
        node.element.classList.contains(node.classIn)
        ...
        debugIf(
          debug,
          `n-view-prompt: node ${node.element.id} is after end: ${node.end}  [removing classIn: ${node.classIn}]`,
        )
      ...
      if (node.classOut) {
        ...
        if (!node.element.classList.contains(node.classOut)) {
          debugIf(
            debug,
            `n-view-prompt: node ${node.element.id} is after end: ${node.end} [adding classOut: ${node.classOut}]`,
          )
        ...
        debugIf(
          debug,
          `n-view-prompt: node ${node.element.id} is after end: ${node.end} [adding hidden attribute]`,
        )
